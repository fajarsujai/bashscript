...
@click.option('--nodetype', prompt='Output nodetype', help='Menambahkan nodetype.')
...
def jf():
    """
    CLI untuk mendownload file JSON dari URL dan menyimpannya ke file lokal.
    """
    try:
        # Mendownload konten dari URL
        response = requests.get("https://raw.githubusercontent.com/fajarsujai/devops-tools-als/main/Jenkinsfile")
        response.raise_for_status()  # Memeriksa jika ada error pada HTTP
        # Membuat folder helminit jika belum ada
        devops_folder = os.path.join(os.getcwd(), 'devops')
        os.makedirs(devops_folder, exist_ok=True)

        # Menyimpan file ke current directory dengan nama 'Jenkinsfile'
        output_path = os.path.join(devops_folder, 'Jenkinsfile')
        with open(output_path, 'w') as file:
            file.write(response.text)

        click.echo(f"Jenkinsfile berhasil didownload dan disimpan di: {output_path}")

    except requests.exceptions.RequestException as e:
        click.echo(f"Error saat mendownload file JSON: {e}")
...
def setval(env):
    ...
    try:
        ...
        yaml_data['volumes'][0]['name'] = volume_name
        yaml_data['volumes'][0]['secret']['secretName'] = secret_name
        yaml_data['volumeMounts'][0]['name'] = volume_name
        yaml_data['volumeMounts'][0]['mountPath'] = '/.env'
        yaml_data['volumeMounts'][0]['subPath'] = '.env'
        yaml_data['volumeMounts'][0]['readOnly'] = read_only
